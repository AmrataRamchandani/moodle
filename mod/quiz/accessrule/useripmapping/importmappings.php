...
if (empty($iid)) {
    ...
    if ($formdata = $mform->is_cancelled()) {
        ...
        $filecolumns = um_validate_user_mapping_columns($cir, $STD_FIELDS, $returnurl);
    } else {
        echo $OUTPUT->header();
...
if ($formdata = $mform2->is_cancelled()) {
    $cir->cleanup(true);
    redirect($returnurl);
} elseif ($formdata = $mform2->get_data()) {
    echo $OUTPUT->header();
    $cir->init();
...
if ($missingusernames == 0 && $missingips == 0 && $bothfieldsmissing == 0 && $countofnotenrolled == 0 && $countofnotregistered == 0) {
